...
function resetGame(){
	...
	for(var i = 0; i < squares.length; ++i){
		//we use this condition cuz undefined it's false. So if there is no color at that position the condition is false:
		if(colors[i]){
...
function pickRandomColor(){
	...
	var random = Math.floor(Math.random() * colors.length); // Math.random() * (max - min) + min === Math.random() * (array.lenght - 0) + 0
	//random numbers between:  [0, array.length)

	return colors[random];
}

//Math.random() generates a random number between 0 and 1.
//Generating a random number in this range: [min, max) 
